...
public class BreadcrumbServiceImplDiffblueTest {
  ...
  @Autowired
  private BreadcrumbServiceImpl breadcrumbServiceImpl;

  /**
   * Test {@link BreadcrumbServiceImpl#buildBreadcrumbDTOs(String, Map)}.
   * <ul>
   *   <li>Given {@link BreadcrumbServiceImpl} (default constructor).</li>
   * </ul>
   * <p>
   * Method under test: {@link BreadcrumbServiceImpl#buildBreadcrumbDTOs(String, Map)}
   */
  @Test
   ...
   * Test {@link BreadcrumbServiceImpl#buildBreadcrumbDTOs(String, Map)}.
   * <ul>
   *   <li>Then calls {@link BreadcrumbServiceExtensionHandler#modifyBreadcrumbList(String, Map, ExtensionResultHolder)}.</li>
   * </ul>
   * <p>
   * Method under test: {@link BreadcrumbServiceImpl#buildBreadcrumbDTOs(String, Map)}
   */
  ...
  public void testBuildBreadcrumbDTOs_thenCallsModifyBreadcrumbList() {
    ...
    when(breadcrumbServiceExtensionHandler.modifyBreadcrumbList(Mockito.<String>any(),
        Mockito.<Map<String, String[]>>any(), Mockito.<ExtensionResultHolder<List<BreadcrumbDTO>>>any()))
        .thenReturn(ExtensionResultStatusType.HANDLED);
    when(breadcrumbServiceExtensionManager.getProxy()).thenReturn(breadcrumbServiceExtensionHandler);

    // Act
    List<BreadcrumbDTO> actualBuildBreadcrumbDTOsResult = breadcrumbServiceImpl
        .buildBreadcrumbDTOs("https://example.org/example", new HashMap<>());

    // Assert
    verify(breadcrumbServiceExtensionHandler).modifyBreadcrumbList(eq("https://example.org/example"), isA(Map.class),
