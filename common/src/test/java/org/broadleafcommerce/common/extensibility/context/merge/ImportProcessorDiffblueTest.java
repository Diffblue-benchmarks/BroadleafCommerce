...
public class ImportProcessorDiffblueTest {
  ...
  public void testNewImportProcessor() {
    // Arrange and Act
    ImportProcessor actualImportProcessor = new ImportProcessor(new AnnotationConfigReactiveWebApplicationContext());
    ...
    Collection<ProtocolResolver> protocolResolvers = ((AnnotationConfigReactiveWebApplicationContext) resourceLoader)
    ...
    ConfigurableListableBeanFactory beanFactory = ((AnnotationConfigReactiveWebApplicationContext) resourceLoader)
        .getBeanFactory();
    ...
    assertTrue(((AnnotationConfigReactiveWebApplicationContext) resourceLoader)
        .getEnvironment() instanceof StandardReactiveWebEnvironment);
    assertEquals("", ((AnnotationConfigReactiveWebApplicationContext) resourceLoader).getApplicationName());
    assertNull(documentBuilder.getSchema());
    assertNull(((AnnotationConfigReactiveWebApplicationContext) resourceLoader).getParentBeanFactory());
    assertNull(((AnnotationConfigReactiveWebApplicationContext) resourceLoader).getParent());
    assertEquals(0L, ((AnnotationConfigReactiveWebApplicationContext) resourceLoader).getStartupDate());
    assertEquals(6, ((AnnotationConfigReactiveWebApplicationContext) resourceLoader).getBeanDefinitionCount());
    assertEquals(6, ((AnnotationConfigReactiveWebApplicationContext) resourceLoader).getBeanDefinitionNames().length);
    ...
    assertTrue(
        ((AnnotationConfigReactiveWebApplicationContext) resourceLoader).getBeanFactoryPostProcessors().isEmpty());
    assertSame(beanFactory,
        ((AnnotationConfigReactiveWebApplicationContext) resourceLoader).getDefaultListableBeanFactory());
  }
   ...
   * Test {@link ImportProcessor#extract(ResourceInputStream[])}.
   * <ul>
   *   <li>When empty array of {@link ResourceInputStream}.</li>
  ...
  @Category(MaintainedByDiffblue.class)
  @MethodsUnderTest({"ResourceInputStream[] ImportProcessor.extract(ResourceInputStream[])"})
  public void testExtract_whenEmptyArrayOfResourceInputStream_thenReturnArrayLengthIsZero() throws MergeException {
    // Arrange, Act and Assert
    assertEquals(0, (new ImportProcessor(new AnnotationConfigReactiveWebApplicationContext()))
        .extract(new ResourceInputStream[]{}).length);
  }
