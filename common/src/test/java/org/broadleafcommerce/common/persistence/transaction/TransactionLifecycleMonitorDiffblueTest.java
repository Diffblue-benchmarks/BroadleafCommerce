...
public class TransactionLifecycleMonitorDiffblueTest {
  @Autowired
  private TransactionLifecycleMonitor transactionLifecycleMonitor;
   ...
   */
  @Test
  public void testDetectExpiry_givenUnsupportedOperationExceptionWithAString() {
    //   Diffblue Cover was unable to create a Spring-specific test for this Spring method.
    ...
    ArrayList<Integer> infosToRemove = new ArrayList<>();
    TransactionInfo info = mock(TransactionInfo.class);
    when(info.getFaultStateDetected()).thenThrow(new UnsupportedOperationException(
        "TRANSACTIONMONITOR(2) - The thread associated with the tested TransactionInfo is not considered stuck"
            + " yet, but the TransactionInfo has been alive for %s milliseconds. This could indicate a overly long"
            + " transaction time. Logging TransactionInfo: \n" + "%s"));
    when(info.getStuckThreadStartTime()).thenReturn(null);
    when(info.getStartTime()).thenReturn(1L);

    // Act
    transactionLifecycleMonitor.detectExpiry(infosToRemove, 1, Long.MAX_VALUE, info,
        new StackTraceElement[]{new StackTraceElement("foo", "foo", "foo", 1)});
