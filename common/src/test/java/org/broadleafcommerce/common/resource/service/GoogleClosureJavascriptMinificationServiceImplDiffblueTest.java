...
public class GoogleClosureJavascriptMinificationServiceImplDiffblueTest {
  ...
  public void testMinifyJs_thenStringWriterToStringIsA() throws ResourceMinificationException {
    // Arrange
   ...
   *
   * <ul>
   *   <li>When {@link StringReader#StringReader(String)} with {@code import UUUfrom "UUU" ;}.
   * </ul>
   *
   * <p>Method under test: {@link GoogleClosureJavascriptMinificationServiceImpl#minifyJs(String,
   * Reader, Writer)}
  ...
  public void testMinifyJs_whenStringReaderWithImportUUUfromUuu()
      throws ResourceMinificationException {
    // Arrange
    StringReader reader = new StringReader("import  UUUfrom  \"UUU\"  ;");
   ...
   *
   * <ul>
   *   <li>When {@link StringReader#StringReader(String)} with {@code UUU= goog.provide (xx)}.
   * </ul>
   *
   * <p>Method under test: {@link GoogleClosureJavascriptMinificationServiceImpl#minifyJs(String,
   * Reader, Writer)}
  ...
  public void testMinifyJs_whenStringReaderWithUuuGoogProvideXx()
      throws ResourceMinificationException {
    // Arrange
    StringReader reader = new StringReader("UUU=  goog.provide  (xx)");
   ...
   * GoogleClosureJavascriptMinificationServiceImpl#compileJs(SourceFile, String)}
   */
  @Test
  @Category(ContributionFromDiffblue.class)
  @ManagedByDiffblue
  @MethodsUnderTest({
    "String GoogleClosureJavascriptMinificationServiceImpl.compileJs(SourceFile, String)"
  })
  public void testCompileJs() throws IOException, ResourceMinificationException {
    ...
    when(input.getCode()).thenReturn("UUU=  goog.provide  (xx)");
  ...
  public void testCompileJs3() throws IOException, ResourceMinificationException {
    // Arrange
    SourceFile input = mock(SourceFile.class);
    SimpleRegion simpleRegion = new SimpleRegion(1, Integer.MIN_VALUE, "Source");
    when(input.getLines(anyInt(), anyInt())).thenReturn(simpleRegion);
    when(input.isExtern()).thenReturn(true);
    doNothing().when(input).clearCachedSource();
    when(input.getCode()).thenReturn("UUU=  goog.provide  (xx)");
    when(input.isWeak()).thenReturn(true);
   ...
   * <ul>
   *   <li>Given {@code Code}.
   *   <li>When {@link SourceFile} {@link SourceFile#getCode()} return {@code Code}.
   ...
   * <ul>
   *   <li>Given cr.
   *   <li>When {@link SourceFile} {@link SourceFile#getCode()} return cr.
   * </ul>
   *
   * <p>Method under test: {@link
   * GoogleClosureJavascriptMinificationServiceImpl#compileJs(SourceFile, String)}
  ...
  public void testCompileJs_givenCr_whenSourceFileGetCodeReturnCr()
      throws IOException, ResourceMinificationException {
    // Arrange
    SourceFile input = mock(SourceFile.class);
    when(input.isExtern()).thenReturn(true);
   ...
   *   <li>When {@link SourceFile} {@link SourceFile#getCode()} throw {@link
  ...
  public void testCompileJs_givenIOException_whenSourceFileGetCodeThrowIOException()
      throws IOException, ResourceMinificationException {
    // Arrange
    SourceFile input = mock(SourceFile.class);
    when(input.getCode()).thenThrow(new IOException());
    when(input.isWeak()).thenReturn(true);
    when(input.getName()).thenReturn("Name");
